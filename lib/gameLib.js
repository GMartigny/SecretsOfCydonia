...
function loadMedia(arr, act){
    ...
    var weight = 0,
        ...
        output = {},
        callback = function(){
            loadMedia.loadingProgress += this.weight;
            act(loadMedia.loadingProgress / sum * 100, this.name);
        };
...
/**
 * Give the type of file based on its extension
 * @param {String} name The file's name
 * @returns {String|Boolean} The file's type as a String, false if no extension is found
 */
...
/**
 * Prepare all canvas inside an element (set their size and get each context)
 * @param {String} holder The holder element's ID
 * @param {Integer|String} width Optional width to set
 * @param {Integer|String} heightOptional height to set
 * @returns {Object} The set of canvas accessible via their IDs<br/>
 * { overlay: { can: <CanvasElement>, ctx: CanvasContext } }
 */
